        ...
        <div className="max-w-5xl mx-auto px-4 py-16">
            <div className="relative space-y-10">
                <div className="flex items-center gap-6">
                    <Image src={sponsor.logo} alt={sponsor.name} width={80} height={80} className="object-contain" />
                    <div>
                ...
                <div className="flex flex-wrap gap-2">
                    {sponsor.sports.map((sport: string) => (
                        <span key={sport} className="text-xs bg-gray-100 px-3 py-1 rounded-full border">
                            {sport}
                        </span>
                    ))}
                </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-6 bg-white border p-6 rounded-lg ">
                    ...
                    {sponsor.socials.instagram && (
                        <div>
                            ...
                            <a href={sponsor.socials.instagram} target="_blank" rel="noopener noreferrer" className="text-sm text-blue-600 hover:underline">
                                {sponsor.socials.instagram}
                            </a>
                        </div>
                    )}
                    {sponsor.socials.facebook && (
                        <div>
                            ...
                            <a href={sponsor.socials.facebook} target="_blank" rel="noopener noreferrer" className="text-sm text-blue-600 hover:underline">
                                {sponsor.socials.facebook}
                            </a>
                        </div>
                    )}
                    {sponsor.socials.twitter && (
                        <div>
                            ...
                            <a href={sponsor.socials.twitter} target="_blank" rel="noopener noreferrer" className="text-sm text-blue-600 hover:underline">
                                {sponsor.socials.twitter}
                            </a>
                        </div>
                    )}
                ...
                <div className="flex gap-4 mt-6 items-center">
                    ...
                    {sponsor.isFollowed && <span className="text-xs px-2 py-1 bg-green-100 text-green-700 rounded-full">Sponsor suivi</span>}
                    {sponsor.isLocal && <span className="text-xs px-2 py-1 bg-blue-100 text-blue-700 rounded-full">Actif dans ma région</span>}
                    {sponsor.applicationsOpen ? (
                        <span className="text-xs px-2 py-1 bg-lime-100 text-lime-700 rounded-full">Ouvert aux candidatures</span>
                    ) : (
                        <span className="text-xs px-2 py-1 bg-gray-100 text-gray-500 rounded-full">Fermé aux candidatures</span>
                    )}
                </div>
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-6">
                            {sponsor.athletes.map((athlete: any, i: number) => (
                                <div key={i} className="relative rounded-xl overflow-hidden group">
                                    <img src={athlete.image} alt={athlete.name} className="w-full h-64 object-cover" />
                ...
                {sponsor.products && sponsor.products.length > 0 && (
                    <div>
                        <h2 className="text-lg font-semibold mt-10 mb-4">Produits phares</h2>
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                            {sponsor.products.map((product: any, i: number) => (
                                <div key={i} className="rounded-lg overflow-hidden">
                                    <Image src={product.image} alt={product.name} width={300} height={200} className="object-contain w-full h-40" />
                                    <div className="p-3">
                ...
                {sponsor.news && sponsor.news.length > 0 && (
                    <div className="mt-12">
                        <h2 className="text-lg font-semibold mb-4">Actualités & événements</h2>
                        <ul className="text-sm text-gray-700 space-y-2">
                            {sponsor.news.map((item: any, i: number) => (
                                <li key={i}>
                                    <span className="font-medium">{item.title}</span> — {item.date}
                                </li>
                            ))}
                        </ul>
                    </div>
